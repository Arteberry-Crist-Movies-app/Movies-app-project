...
//Loading message code
    ...
    loadingMessage.innerHTML = `<iframe src="https://giphy.com/embed/Q0cwjn4FS474gO04uO" width="100%" height="480" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="https://giphy.com/stickers/CampariIT-venezia-campari-venezia79-Q0cwjn4FS474gO04uO">via GIPHY</a></p>`;
...
//Creating a way to display the movies on HTML
                ...
                const movieCard = `<div class="movie-card" id="movie-${movie.id}"><h3>${name}</h3><p id="movie-img">${poster}</p><p>Voter Rating: ${rating}</p></div>`;
//Displaying the movies on HTML
                document.querySelector('#movies').innerHTML += movieCard;
                addMovieToJSON(movie);
            }
        })
        .catch(err => {
            console.error(err);
